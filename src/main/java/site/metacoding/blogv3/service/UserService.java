...
public class UserService {
    ...
    public void 회원가입(User user) {
        ...
        String encPassword = bCryptPasswordEncoder.encode(rawPassword); // Hash 알고리즘 사용
    ...
    public void 패스워드초기화(PasswordResetReqDto passwordResetReqDto) {
        ...
        // 2. 같은 것이 있다면 DB password 초기화 - BCrypt 해시 - update하기 (DB)
        if (userOp.isPresent()) {
            User userEntity = userOp.get(); // 영속화
            String encPassword = bCryptPasswordEncoder.encode("9999"); // 파일에 넣어서 파일을 읽어서 넣어야 한다
